...
fn add_padding_symbols(obj: &mut ObjInfo) -> Result<()> {
    ...
    for (section_index, section) in obj.sections.iter() {
        ...
        let mut to_add = vec![];
        let section_start_addr = section.address as u32;
        let section_end_addr = section_start_addr.wrapping_add(section.size as u32);
        let mut iter = obj
            .symbols
            .for_section(section_index)
            .filter(|(_, s)| s.size_known && s.size > 0)
            // Skip symbols whose declared range escapes the section. On
            // Xbox 360 XEX inputs, symbols.txt (sometimes hand-curated or
            // ported from a sibling build) can list labels that fall in
            // the file-alignment padding past a section's virtual end —
            // e.g. lbl_821E99C0 declared as .rdata when .rdata ends at
            // 0x821E99AC. Including them in the gap-detection iteration
            // makes data_range() bail with "Range … outside of section
            // …", killing the whole split with one stale label.
            .filter(|(_, s)| {
                let addr = s.address as u32;
                let end = addr.wrapping_add(s.size as u32);
                let in_bounds =
                    addr >= section_start_addr && end <= section_end_addr && end >= addr;
                if !in_bounds {
                    log::warn!(
                        "Skipping symbol {} @ {:#010X}..{:#010X}: outside section {} ({:#010X}..{:#010X})",
                        s.name, addr, end, section.name, section_start_addr, section_end_addr,
                    );
                }
                in_bounds
            })
...
fn split_pdata(obj: &mut ObjInfo) -> Result<()> {
    // Clear all existing pdata splits to avoid duplicates and false "overlaps with
    // split" errors when split_pdata is called multiple times.
...
mod tests {
    ...
    /// ObjSplits::clear() should remove all splits.
    #[test]
    fn test_obj_splits_clear() {
        use crate::obj::ObjSplits;

        let mut splits = ObjSplits::default();
        splits.push(0x1000, ObjSplit {
            unit: "A".into(),
            end: 0x1080,
            align: None,
            common: false,
        ...
        assert_eq!(splits.iter().count(), 2);
        splits.clear();
        assert_eq!(splits.iter().count(), 0);
        assert!(!splits.has_split_at(0x1000));
        assert!(!splits.has_split_at(0x1080));
    }

    /// split_pdata() should clear pre-existing pdata splits before regenerating,
    /// avoiding duplicates when called multiple times.
    #[test]
    fn test_split_pdata_clears_existing_splits() {
        // Build: .text at 0x1000 (two functions), .pdata at 0x2000
        let text_data = vec![0x60u8; 0x100];
        let mut text_sec = make_section(".text", ObjSectionKind::Code, 0x1000, text_data);

        // .pdata: two 8-byte entries referencing 0x1000 and 0x1080
        let mut pdata_data = vec![0u8; 0x10];
        pdata_data[0..4].copy_from_slice(&0x1000u32.to_be_bytes());
        pdata_data[8..12].copy_from_slice(&0x1080u32.to_be_bytes());
        let mut pdata_sec =
        ...
        // Pre-populate .pdata with a stale split that should be cleared
        pdata_sec.splits.push(0x2000, ObjSplit {
            unit: "StaleUnit".into(),
            end: 0x2010,
            align: None,
        ...
        // Verify stale split exists before
        let pdata_sec_before = &obj.sections[1];
        assert_eq!(
            pdata_sec_before.splits.iter().count(),
            1,
            "Should have 1 stale split before"
        );
        assert_eq!(
            pdata_sec_before.splits.iter().next().unwrap().1.unit,
            "StaleUnit"
        );

        // Run split_pdata — it should clear the stale split and create new ones
        split_pdata(&mut obj).unwrap();

        // Verify stale split is gone and new splits are present
        let pdata_sec_after = &obj.sections[1];
        let splits: Vec<_> = pdata_sec_after.splits.iter().collect();

        // Should have splits for UnitA and UnitB, not StaleUnit
        assert!(
            !splits.iter().any(|(_, s)| s.unit == "StaleUnit"),
            "StaleUnit should have been cleared"
        );
        assert_eq!(splits.len(), 2, "Should have 2 pdata splits (one per code split)");

        let units: Vec<_> = splits.iter().map(|(_, s)| s.unit.as_str()).collect();
        assert!(units.contains(&"UnitA"), "Should have UnitA split");
        assert!(units.contains(&"UnitB"), "Should have UnitB split");
    }

    /// split_pdata() called twice should produce identical results (idempotent).
    #[test]
    fn test_split_pdata_idempotent() {
        let text_data = vec![0x60u8; 0x100];
        let mut text_sec = make_section(".text", ObjSectionKind::Code, 0x1000, text_data);
        ...
        // Call twice
        split_pdata(&mut obj).unwrap();
        let first_splits: Vec<_> = obj.sections[1]
            .splits
            .iter()
            .map(|(addr, s)| (addr, s.unit.clone(), s.end))
            .collect();

        split_pdata(&mut obj).unwrap();
        let second_splits: Vec<_> = obj.sections[1]
            .splits
            .iter()
            .map(|(addr, s)| (addr, s.unit.clone(), s.end))
            .collect();
        ...
        assert_eq!(
            ...
            "split_pdata should be idempotent — same result on second call"
        );
    }
}
